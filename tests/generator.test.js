  ...
  describe("Main Generation Function", () => {
    describe("generateWorkoutVideo", () => {
      test("should validate input parameters", async () => {
        const invalidParams = {
          workDuration: 5, // Too low
          restDuration: 3, // Too low
          setsPerStation: 0, // Too low
          stationRest: 3, // Too low
          totalWorkoutDuration: 2, // Too low
          categories: ["strength"],
          equipment: ["barbell"],
        };
        ...
        await expect(
          generator.generateWorkoutVideo(
            invalidParams.workDuration,
            invalidParams.restDuration,
            invalidParams.setsPerStation,
      ...
      test("should generate workout video successfully", async () => {
        ...
        // Mock getExerciseVideosByEquipment to return some videos
        const originalGetVideos = generator.getExerciseVideosByEquipment;
        generator.getExerciseVideosByEquipment = jest.fn().mockReturnValue({
          barbell: ["/path/to/exercise1.mp4", "/path/to/exercise2.mp4"],
        });

        // Mock selectExercisesEvenly to return selected exercises
        const originalSelectExercises = generator.selectExercisesEvenly;
        generator.selectExercisesEvenly = jest
          .fn()
          .mockReturnValue(["/path/to/exercise1.mp4"]);

        const result = await generator.generateWorkoutVideo(
          30, // workDuration
          10, // restDuration
          2, // setsPerStation
          15, // stationRest
        ...
        expect(typeof result).toBe("string");
        expect(result).toContain("workout_video");
        expect(result).toContain(".mp4");

        // Restore original functions
        generator.getExerciseVideosByEquipment = originalGetVideos;
        generator.selectExercisesEvenly = originalSelectExercises;
      });
    });
  });
