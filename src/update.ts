...
/**
 * Fetch the latest version from the remote package.json (main branch)
 */
async function fetchLatestVersionFromRemotePackageJson(): Promise<
  string | null
> {
  const url =
    "https://raw.githubusercontent.com/lachlanwp/Wurqit/refs/heads/main/package.json";
      ...
      https
        .get(requestUrl, (response) => {
          ...
          if (
            ...
            console.error(
              "Too many redirects while fetching remote package.json"
            );
            resolve(null);
            return;
          }
          ...
          if (
            ...
            console.error(
              `Failed to fetch remote package.json: HTTP ${response.statusCode}: ${response.statusMessage}`
            );
            resolve(null);
            return;
          }
          ...
          response.on("end", () => {
            try {
              const parsed: PackageJson = JSON.parse(body);
              const version = parsed?.version?.trim();
              if (version) {
                resolve(version);
                return;
              }
              console.error("Remote package.json missing version");
              resolve(null);
            } catch (err) {
              console.error("Error parsing remote package.json:", err);
              resolve(null);
            }
          });
        })
        .on("error", (err) => {
          console.error("Network error fetching remote package.json:", err);
          resolve(null);
        });
    };
      ...
      https
        .get(requestUrl, (response) => {
          ...
          if (
            ...
            if (location) {
              console.log(
                `Following redirect ${redirectCount} to: ${location}`
              );
              ...
              reject(
                new Error(
                  `HTTP ${response.statusCode}: No redirect location found`
                )
              );
              return;
            }
          ...
          response.on("data", (chunk) => {
            ...
            if (onProgress && totalSize > 0) {
              ...
              console.log(
                `Download progress: ${progress}% (${downloadedSize}/${totalSize} bytes)`
              );
              onProgress(progress);
            }
          });
...
export async function checkForUpdates(): Promise<void> {
  try {
    ...
    // Fetch the latest version from remote package.json
    const fetched = await fetchLatestVersionFromRemotePackageJson();
    const latestVersion = fetched || currentVersion;
